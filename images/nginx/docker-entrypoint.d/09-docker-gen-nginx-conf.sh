...
docker_gen_nginx_conf() {
  ...
  entrypoint_log "Backing up $conffile to $conffile.default"

  cp "$conffile" "$conffile.default"

  entrypoint_log "Running docker-gen on $conffile_tmpl to $conffile"
...
docker_gen_nginx_template_conf() {
  ...
  find "$template_dir" -follow -type f -name "*$suffix" -print | while read -r template; do
    ...
    entrypoint_log "Running docker-gen on $template to $output_path"
