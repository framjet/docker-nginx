...
add_stream_block() {
  ...
  if grep -q -E "\s*stream\s*\{" "$conffile"; then
    entrypoint_log "$conffile contains a stream block; include $stream_output_dir/*.conf to enable stream templates"
  else
    # check if the file can be modified, e.g. not on a r/o filesystem
    touch "$conffile" 2>/dev/null || { entrypoint_log "info: can not modify $conffile (read-only file system?)"; exit 0; }
    entrypoint_log "Appending stream block to $conffile to include $stream_output_dir/*.conf"
    cat << END >> "$conffile"
# added by "$ME" on "$(date)"
stream {
...
auto_envsubst() {
  ...
  if [ ! -w "$output_dir" ]; then
    entrypoint_error "$template_dir exists, but $output_dir is not writable"
    return 0
  fi
  ...
  if test -n "$(find "$template_dir" -name "*$stream_suffix" -print -quit)"; then
    ...
    if [ ! -w "$stream_output_dir" ]; then
      entrypoint_log "ERROR: $template_dir exists, but $stream_output_dir is not writable"
      return 0
    fi
    add_stream_block
